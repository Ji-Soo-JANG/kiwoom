...
- [ ] PostgreSQL을 도입해 관심 종목과 포트폴리오를 영구 저장한다.
  - 로컬 개발용 Docker Compose와 운영 환경변수를 구성한다.
  - 스키마 마이그레이션 도구로 Flyway 또는 Liquibase를 적용한다.
  - 서버 재시작 후에도 등록 데이터가 유지되는 통합 테스트를 추가한다.
- [ ] 매수·매도 거래 내역을 저장하고 평균 매입가를 자동 계산한다.
  - 실현 손익과 미실현 손익을 분리한다.
  - 수수료와 세금을 설정 가능하게 반영한다.
