...
- [ ] 백엔드·프론트 통합 실행용 Dockerfile을 추가한다.
- [ ] 운영용 Docker Compose 또는 배포 명세를 작성한다.
  - API 키와 secret은 이미지에 포함하지 않고 Secret 또는 환경변수로 주입한다.
  - 운영 프로필, CORS origin, JVM 메모리 제한을 설정한다.
- [ ] HTTPS와 리버스 프록시를 구성하고 실제 서버에 배포한다.
  - 도메인, 방화벽, 8080 포트 외부 노출 여부를 결정한다.
  - 배포 후 `/actuator/health`와 주요 화면을 확인한다.
...
- [ ] 일봉 조회에 종목·기준일 단위 TTL 캐시를 적용한다.
  - 현재가와 일봉의 캐시 TTL을 독립적으로 설정한다.
  - 캐시 적중률과 실제 키움 API 호출 횟수를 측정한다.
- [ ] 다중 인스턴스 배포가 필요해지면 로컬 캐시를 Redis로 전환한다.
- [ ] 키움 오류 코드를 애플리케이션 오류 코드로 세분화한다.
  - 인증 만료, 호출 제한, 종목 없음, 장 운영시간 오류를 구분한다.
  - 프론트엔드에 오류 유형별 안내와 재시도 동작을 제공한다.
- [ ] Micrometer·Prometheus 기반 운영 지표를 추가한다.
  - API 응답 시간, 실패율, 401 갱신, 429 재시도, 캐시 적중률을 수집한다.
- [ ] 요청 추적 ID와 구조화 JSON 로그를 도입한다.
  - API 키, secret, 접근 토큰이 로그에 포함되지 않는지 자동 검사한다.
- [ ] 목표가와 기술적 지표 조건에 따른 알림 기능을 설계한다.
  - 이메일, Slack 또는 앱 알림 채널 중 하나를 먼저 구현한다.
