...
public class AlertRepository {
    ...
    public AlertRepository(DatabaseClient database) {
    ...
    public Mono<AlertRule> addRule(String username, AlertRuleRequest request) {
        ...
        spec =
                request.threshold() == null
                        ? spec.bindNull("threshold", BigDecimal.class)
                        : spec.bind("threshold", request.threshold());
    ...
    public Mono<AlertRule> updateRule(
            String username, long id, BigDecimal threshold, boolean enabled) {
        DatabaseClient.GenericExecuteSpec spec =
                database.sql(
                                """
        ...
        spec =
                threshold == null
                        ? spec.bindNull("threshold", BigDecimal.class)
                        : spec.bind("threshold", threshold);
        return spec.fetch()
                .rowsUpdated()
                .flatMap(updated -> updated > 0 ? findRule(username, id) : Mono.empty())
                .onErrorMap(
                        DuplicateKeyException.class,
                        error -> new IllegalArgumentException("동일한 목표가 알림 규칙이 이미 존재합니다"));
    }
    ...
    public Mono<AlertEvent> addEvent(String username, AlertRule rule, BigDecimal observedValue) {
        ...
        DatabaseClient.GenericExecuteSpec spec =
                ...
                """)
                        ...
                        .bind("username", username)
                        .bind("code", rule.code())
                        .bind("type", rule.conditionType().name())
                        .bind("observed", observedValue)
                        .bind("triggeredAt", triggeredAt);
        spec =
                rule.threshold() == null
                        ? spec.bindNull("threshold", BigDecimal.class)
                        : spec.bind("threshold", rule.threshold());
        return spec.filter(statement -> statement.returnGeneratedValues("id"))
                .map(
                        row ->
                                new AlertEvent(
    ...
    private AlertRule mapRule(Row row) {
        return new AlertRule(
                ...
                row.get("threshold", BigDecimal.class),
    ...
    private AlertEvent mapEvent(Row row) {
        return new AlertEvent(
                row.get("id", Long.class),
                row.get("rule_id", Long.class),
                row.get("code", String.class),
                AlertConditionType.valueOf(row.get("condition_type", String.class)),
                row.get("observed_value", BigDecimal.class),
                row.get("threshold", BigDecimal.class),
                row.get("triggered_at", OffsetDateTime.class),
                row.get("read_at", OffsetDateTime.class));
    }
}
