...
public class PortfolioTradeRepository {
    ...
    public Mono<PortfolioTrade> save(String username, PortfolioTrade trade) {
        return database.sql("""
                ...
                .map(row -> new PortfolioTrade(row.get("id", Long.class), trade.code(), trade.type(),
                        trade.quantity(), trade.price(), trade.fee(), trade.tax(),
                        trade.realizedProfitLoss(), trade.tradedAt()))
                .one();
    }
    ...
    public Flux<PortfolioTrade> findAll(String username) {
        return database.sql("""
                ...
                """).bind("username", username).map((row, metadata) -> new PortfolioTrade(
                        ...
                        row.get("fee", BigDecimal.class), row.get("tax", BigDecimal.class),
                        row.get("realized_profit_loss", BigDecimal.class),
                        row.get("traded_at", OffsetDateTime.class))).all();
    }
}
