...
function Portfolio({positions, valuations, loading, onSave, onRemove, onValuate}) {
    ...
    const summary = valuations.reduce((total, item) => ({
        purchaseAmount: total.purchaseAmount + Number(item.purchaseAmount ??
            Number(item.averagePrice) * Number(item.quantity)),
    ...
    const totalReturnRate = summary.purchaseAmount === 0 ? 0
        : summary.profitLoss / summary.purchaseAmount * 100;

    return <section className="portfolio" aria-labelledby="portfolio-title">
        <h2 id="portfolio-title">포트폴리오</h2>
        <form className="portfolio-form" onSubmit={submit} aria-label={editingCode ? `${editingCode} 포지션 수정` : '포지션 등록'}>
            ...
            <button type="submit" disabled={loading}>{editingCode ? '수정 저장' : '저장'}</button>
            {editingCode && <button type="button" onClick={clearForm} disabled={loading}>수정 취소</button>}
        </form>
        ...
        {positions.length === 0 ? <p>등록된 보유 종목이 없습니다.</p> : <ul className="portfolio-list">
            {positions.map((position) => <li key={position.code}>
                ...
                <button type="button" onClick={() => edit(position)}
                        aria-label={`${position.code} 포트폴리오 수정`}>수정</button>
        ...
        {valuations.length > 0 && <>
            <section className="portfolio-summary" aria-labelledby="portfolio-summary-title">
                ...
                <dl>
                    <div><dt>총 매입금액</dt><dd>{formatNumber(summary.purchaseAmount)}원</dd></div>
                    <div><dt>총 평가금액</dt><dd>{formatNumber(summary.evaluationAmount)}원</dd></div>
                    <div><dt>총 손익</dt><dd className={summary.profitLoss >= 0 ? 'price-up' : 'price-down'}>{formatNumber(summary.profitLoss)}원</dd></div>
                    <div><dt>총 수익률</dt><dd>{formatNumber(totalReturnRate.toFixed(2))}%</dd></div>
                </dl>
            </section>
            <div className="valuation-table-wrapper">
            <table className="valuation-table">
                <caption>현재가 기준 포트폴리오 평가</caption>
                <thead><tr><th>종목</th><th>현재가</th><th>평가금액</th><th>비중</th><th>손익</th><th>수익률</th></tr></thead>
                <tbody>{valuations.map((item) => <tr key={item.code}>
                    ...
                    <td>{summary.evaluationAmount === 0 ? '0' :
                        (Number(item.evaluationAmount) / summary.evaluationAmount * 100).toFixed(1)}%</td>
