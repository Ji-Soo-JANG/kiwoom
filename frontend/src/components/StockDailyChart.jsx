...
function Candle({ x, y, width, height, payload }) {
  ...
  return (
    <g
      aria-label={`${payload.date} 시가 ${number(open)} 고가 ${number(high)} 저가 ${number(low)} 종가 ${number(close)}`}
    >
      <line x1={center} x2={center} y1={y} y2={y + height} stroke={color} />
      <rect
        x={x + width * 0.2}
        y={bodyTop}
        width={Math.max(width * 0.6, 1)}
...
const PriceTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="chart-tooltip">
      ...
      <span>
        시 {number(item.openPrice)} · 고 {number(item.highPrice)}
      </span>
      <span>
        저 {number(item.lowPrice)} · 종 {number(item.closePrice)}
      </span>
  ...
  const enriched = useMemo(
    () =>
      addTechnicalIndicators(dailyPrices).map((item) => ({
        ...item,
        priceRange: [Number(item.lowPrice), Number(item.highPrice)]
      })),
    [dailyPrices]
  );
  ...
  return (
    <section className="chart-section" aria-labelledby="daily-chart-title">
      ...
      <p className="chart-indicators">
        RSI(14): {latest.rsi == null ? '-' : latest.rsi.toFixed(2)} · MACD:{' '}
        {latest.macd == null ? '-' : latest.macd.toFixed(2)}
      </p>
      ...
      <div className="chart-wrapper chart-volume-panel">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} syncId="stock-daily">
            ...
            <YAxis width={78} tickFormatter={number} />
      ...
      <div className="chart-wrapper chart-indicator-panel">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} syncId="stock-daily">
            <XAxis {...commonXAxis} hide />
            <YAxis width={78} domain={[0, 100]} ticks={[30, 50, 70]} />
      ...
      <div className="chart-wrapper chart-indicator-panel">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} syncId="stock-daily">
            <XAxis {...commonXAxis} />
            <YAxis width={78} domain={['auto', 'auto']} tickFormatter={number} />
