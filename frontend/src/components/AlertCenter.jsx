...
const conditionLabels = {
  PRICE_ABOVE: '목표가 이상',
  PRICE_BELOW: '목표가 이하',
  CHANGE_RATE_ABOVE: '일간 급등',
  CHANGE_RATE_BELOW: '일간 급락',
  RSI_ABOVE: 'RSI 이상',
  RSI_BELOW: 'RSI 이하',
  MACD_CROSS_UP: 'MACD 상향 교차',
  MACD_CROSS_DOWN: 'MACD 하향 교차'
};
...
const thresholdLabel = (type) =>
  type.startsWith('CHANGE_RATE_')
    ? '변동률 (%)'
    : type.startsWith('RSI_')
      ? 'RSI 기준값'
      : '목표가';
const formatRuleValue = (type, value) =>
  `${formatPrice(value)}${type.startsWith('CHANGE_RATE_') ? '%' : ''}`;
...
function AlertCenter({ onError }) {
  ...
  return (
    <section className="alert-center" aria-labelledby="alert-title">
      <div className="alert-heading">
        <h2 id="alert-title">주가·지표 알림</h2>
        {unreadCount > 0 && (
          <span className="alert-badge" aria-label={`읽지 않은 알림 ${unreadCount}개`}>
            {unreadCount}
          </span>
        )}
      </div>
      {(rulesQuery.error || eventsQuery.error) && (
      ...
      <form className="alert-form" onSubmit={submit}>
        ...
        <input
          ...
          min={
            conditionType.startsWith('RSI_')
              ? '0'
              : isPercentThreshold(conditionType)
                ? '0.01'
                : '1'
          }
          max={isPercentThreshold(conditionType) ? '100' : undefined}
          step="any"
          required={requiresThreshold(conditionType)}
      ...
      {rules.length === 0 ? (
        ...
        <ul className="alert-list">
          {rules.map((rule) => (
            <li className="alert-rule-row" key={rule.id}>
              <span>
                {rule.code} · {conditionLabels[rule.conditionType]}
                {rule.threshold == null
                  ? ''
                  : ` ${formatRuleValue(rule.conditionType, rule.threshold)}`}
              </span>
      ...
      {events.length === 0 ? (
        ...
        <ul className="alert-list" aria-live="polite">
          {events.map((item) => (
            <li className={item.readAt ? '' : 'alert-event-unread'} key={item.id}>
              <div className="alert-event-row">
                <span>
                  {item.code} · {conditionLabels[item.conditionType]} · 관측값{' '}
                  {formatRuleValue(item.conditionType, item.observedValue)}
                  {item.threshold == null
                    ? ''
                    : ` / 기준 ${formatRuleValue(item.conditionType, item.threshold)}`}
                </span>
                {!item.readAt && (
                  <button type="button" onClick={() => markRead(item.id)} disabled={loading}>
                    읽음
                  </button>
                )}
              </div>
