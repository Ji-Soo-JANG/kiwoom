...
const dailyPrices = Array.from({length: 40}, (_, index) => ({
  date: index < 30 ? `202606${String(index + 1).padStart(2, '0')}`
    : `202607${String(index - 29).padStart(2, '0')}`,
  openPrice: 70000 + index * 100, highPrice: 71000 + index * 100,
  lowPrice: 69000 + index * 100, closePrice: 70500 + index * 100,
  volume: 100000 + index, rsi: 50, macd: 10, signal: 8
}));

test('종목을 검색하고 관심 종목에 추가한다', async ({page}) => {
  await mockApi(page, {
    'GET /api/kiwoom/stock-price/005930': (route) => json(route, {
      code: '005930', currentPrice: '75000', changeAmount: '500', changeRate: '0.67'
    }),
...
test('포트폴리오를 등록하고 현재가로 평가한다', async ({page}) => {
  await mockApi(page, {
    'GET /api/portfolio/valuation': (route) => json(route, [{
      code: '005930', quantity: 10, averagePrice: 70000, purchaseAmount: 700000,
      currentPrice: 75000, evaluationAmount: 750000, profitLoss: 50000, returnRate: 7.14
    }])
  });
